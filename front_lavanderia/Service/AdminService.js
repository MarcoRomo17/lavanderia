  ...
  useEffect(() => {
    buscarEnLaBD()
      ...
      const buscarEnLaBD=async()=>{ 
        try {
          const Services = await axios.get("https://dh8j0891-5000.usw3.devtunnels.ms/services/get-all")          
          setdatosTabla(Services.data.found)
        } catch (error) {
      ...
      const eliminarCliente= async(ID)=>{// Esta es la funcion para eliminar. Recibe de parametro el id del cliente que vamos a eliminar. Es llamada desde los botones de la tabla
          ...
          try {//el trycatch
           await axios.delete(`https://dh8j0891-5000.usw3.devtunnels.ms/services/delete/${ID}`)//como puedes ver, por la naturaleza del endpoint de como mandarle el id, el texto de la URL debe ser dinamico
    ...
    const mapiado= datosTabla.map((Registro)=>([Registro.name,Registro.description,String(Registro.price),
      (<>
