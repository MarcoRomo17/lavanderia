...
export const Allorders =()=>{ 
    const [DashboardallOrders, setDashboardallOrders] = useState([]);
    ...
    try {
      console.log("Hola, soy la paginacion", Paginacion)
      const allOrders= await axios.get(`https://dh8j0891-5000.usw3.devtunnels.ms/orders/get-orders-dashboard?pagination=${Paginacion}`)
     setDashboardallOrders(allOrders.data)
    } catch (error) {
      console.log(error)
    ...
    const {navigate}= useNavigation()// esta madrecita es la que nos permitira navegar entre paginas
        ...
        const mapiado= DashboardallOrders.map((Registro)=>([Registro.user_name,Registro.client_name,Registro.created_at,Registro.state,Registro.total,
         (<>
